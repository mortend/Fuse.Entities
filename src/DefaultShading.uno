...
namespace Fuse.Entities
{
	public block DefaultShading
	{
		...
		public Sphere BoundingSphere: prev;
		public Sphere WorldBoundingSphere: new Sphere(Transform(float4(BoundingSphere.Center, 1), World).XYZ, BoundingSphere.Radius * Math.ComponentMax(Math.Abs(Matrix.GetScaling(World))));

		public float LinearDepth:
			(-ViewPosition.Z - CameraDepthRange[0]) / (CameraDepthRange[1] - CameraDepthRange[0]);
		...
		public float4x4 World:
			tag("Identity") req(WorldSrtMatrix tag "Identity") float4x4.Identity,
			tag("UniformScale") req(WorldSrtMatrix tag "UniformScale") WorldSrtMatrix,
			WorldSrtMatrix;

		World:
			req(prev prev World as float4x4) Mul(prev, prev prev World),
			prev;
		...
		*/
		...
		public float3 TangentNormal: NormalMap != null ?
			Normalize(sample(NormalMap, TexCoord, SamplerState.LinearWrap).XYZ * 2.0f - 1.0f) : float3(0,0,1);
		...
		public float4 DiffuseMapColor: DiffuseMap != null ?
			sample(DiffuseMap, TexCoord, SamplerState.LinearWrap) : float4(1);
		...
		public float4 SpecularMapColor: SpecularMap != null ?
			sample(SpecularMap, TexCoord, SamplerState.LinearWrap) : float4(1);
